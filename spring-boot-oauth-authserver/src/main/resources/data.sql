insert into user (id,name,account_non_expired,account_non_locked,created_at,credentials_non_expired,enabled,password,updated_at,username)
 values (null,'admin-lms',1,1,current_timestamp(),1,1,'$2a$10$8.UnVuG9HHgffUDAlk8qfOuVGkqRzgVymGe07xd00DMxs.AQubh4a',current_timestamp(),'admin');

insert into boot_oauth_server.role (id,name) values (null, 'ROLE_ADMIN');
insert into boot_oauth_server.role (id,name) values (null, 'ROLE_USER');

insert into boot_oauth_server.user_role (user_id,role_id) values (1, 1);
insert into boot_oauth_server.user_role (user_id,role_id) values (1, 2);
